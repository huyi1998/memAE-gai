...
parser.add_argument('--dataset', type=str, default='factory', help="Which dataset am I working on? Avenue, UCSDped2")
parser.add_argument('--datapath', type=str, default='anomaly_data/factory', help="Data directory")
...
def save_factory_frame(use_str, path2read):
    path2write = path2read + "/frames/%s" % use_str
    path2read = path2read + "/%s_videos" % use_str
    print("Reading avenue dataset from ", path2read)
    print("Saving the frames in", path2write)
    ...
    for iterr, single_child in enumerate(path_child):
        video_path = path2read + '/' + single_child
        if '.avi' in single_child:
            s_video_name = single_child.split('.avi')[0]
        elif '.MOV' in single_child:
            s_video_name = single_child.split('.MOV')[0]
        elif '.mov' in single_child:
            s_video_name = single_child.split('.mov')[0]
        elif '.mp4' in single_child:
            s_video_name = single_child.split('.mp4')[0]
        cap = cv2.VideoCapture(video_path)
        i = 0
        while (cap.isOpened()):
            ret, frame = cap.read()
            # 每个视频的帧数不同，当frame是空（0btye）时,也会出现这个错误
            try:
                # 原视频分辨率太大，切换原视频分辨率
...
if __name__ == "__main__":
    ...
    if args.dataset == "Avenue":
        ...
        save_factory_frame("training", args.datapath)
        save_factory_frame("testing", args.datapath)

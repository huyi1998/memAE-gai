...
def main():
    ...
    parser.add_argument('--version', type=int, default=6, help='experiment version')
    ...
    for epoch in range(current_epoch, args.epochs):
        model.train()
        tr_re_loss, tr_mem_loss, tr_tot, tr_trip_loss = 0.0, 0.0, 0.0, 0.0
        ...
        for batch_idx, frame in enumerate(progress_bar):
            ...
            recons, recons_bad, attr = model_output['output_good'], model_output['output_bad'], model_output['att']
            trip_loss = loss.triplet_loss(target_frame, recons, recons_bad)
            ...
            tot_loss = re_loss/re_loss.detach() + mem_loss/(mem_loss+1e-6) + trip_loss/trip_loss.detach()  # version 6
            ...
            tr_trip_loss += trip_loss.data.item()
        ...
        train_writer.add_scalar("model/train-trip-loss", tr_trip_loss / len(train_batch), epoch)
        ...
        if epoch % args.val_epoch == 0:
            ...
            re_loss_val, mem_loss_val, trip_loss_val = 0.0, 0.0, 0.0
            for batch_idx, frame in enumerate(test_batch):
                ...
                recons, recons_bad, attr = model_output['output_good'], model_output['output_bad'], model_output['att']
                trip_loss = loss.triplet_loss(target_frame, recons, recons_bad)
                # recons, attr = model_output['output_good'], model_output['att']
                ...
                trip_loss_val += trip_loss.data.item()
            ...
            train_writer.add_scalar("model/val-trip-sparse", tr_trip_loss / len(test_batch), epoch)
            print("epoch %d" % epoch, "recons loss training %.4f validation %.4f" % (tr_re_loss, re_loss_val),
                  "memory sparsity training %.4f validation %.4f trip loss %.4f" % (tr_mem_loss, mem_loss_val, trip_loss_val))

...
def main():
    ...
    parser.add_argument('--EntropyLossWeight', help='EntropyLossWeight', type=float, default=0)
    ...
    parser.add_argument('--version', type=int, default=2, help='experiment version')
    ...
    for epoch in range(current_epoch, args.epochs):
        ...
        for batch_idx, frame in enumerate(progress_bar):
            ...
            if args.flownet == 'lite':
                # input_last,target_frame=loss.get_unormalized_data(input_frame, recons, 0.5, 0.5)
                gt_flow_input = torch.cat([input_last, target_frame], 1)
                pred_flow_input = torch.cat([input_last, recons], 1)
            ...
            fl_l = loss.flow_loss(flow_pred, flow_gt)

            # frame和recon: [16,1,8,256,256]
            ...
            tot_loss = 1. * re_loss + 2. * fl_l + mem_loss * args.EntropyLossWeight
        ...
        #         # recons, attr = model_output['output'], model_output['att']
        #         recons = model_output
        #         re_loss = loss.get_reconstruction_loss(frame, recons, mean=0.5, std=0.5)  # 公式8
        #         # mem_loss = loss.get_memory_loss(attr)  # 公式9
        #         # tot_loss = re_loss + mem_loss * args.EntropyLossWeight  # 公式10
        #         tr_re_loss += re_loss.data.item()  # .data()-将variable变量变为tensor;  .item()-将tensor变为float
        #         # tr_mem_loss += mem_loss.data.item()
